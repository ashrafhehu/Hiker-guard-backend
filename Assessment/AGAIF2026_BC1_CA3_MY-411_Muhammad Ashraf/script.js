...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Offline Blank Grid Canvas Layer
  ...
  // ------------------------------------------------------------------------
  // 4. Country Boundary Polygon Layer Styling & Interaction (Bold Crisp Borders)
  // ------------------------------------------------------------------------
  ...
  function styleCountryFeature(feature) {
    ...
    const color = getCountryColor(cName);
    return {
      fillColor: color,
      weight: 2.5,
      opacity: 1.0,
      color: '#ffffff', // Bright crisp white border stroke for maximum visibility!
      dashArray: '',
      fillOpacity: 0.45 // Rich opacity for bold country polygons
    };
  }
  ...
  function highlightCountry(e) {
    ...
    layer.setStyle({
      weight: 4.0,
      color: '#fbbf24', // Gold border glow on hover!
      dashArray: '',
      fillOpacity: 0.70
    });
  ...
  const countriesLayer = L.geoJSON(countriesData, {
    ...
    onEachFeature: (feature, layer) => {
      ...
      layer.bindTooltip(`<strong>${cName}</strong> (ASEAN Member)`, {
        sticky: true,
        direction: 'auto',
  ...
  const overlayMaps = {
    "ASEAN Country Boundaries (Polygons)": countriesLayer,
    "ASEAN Municipal Places (Points)": placesLayer
  };
